...
export function AuthMorphLayout({ rightPanelActive = false, signInForm, signUpForm }: AuthMorphLayoutProps) {
  ...
  const transitionSettings: Transition = { duration: 0.7, ease: 'easeInOut' }
        ...
        >
          ...
          >
            <motion.div
              ...
              transition={transitionSettings}
            >
              <h2 className="mb-5 text-4xl font-bold tracking-wide">{t('auth.welcome_back', {}, 'Welcome Back!')}</h2>
              <p className="mb-10 px-4 text-[14px] font-medium leading-relaxed text-primary-foreground/90">
                {t('auth.keep_connected_message', {}, 'To keep connected with us please login with your personal info')}
              </p>
              <a
                href={route('login')}
            ...
            <motion.div
              ...
              transition={transitionSettings}
            >
              <h2 className="mb-5 text-4xl font-bold tracking-wide">{t('auth.hello_friend', {}, 'Hello, Friend!')}</h2>
              <p className="mb-10 px-4 text-[14px] font-medium leading-relaxed text-primary-foreground/90">
                {t('auth.register_message', {}, 'Register with your personal details to use all of site features')}
              </p>
              <a
                href={route('register')}
